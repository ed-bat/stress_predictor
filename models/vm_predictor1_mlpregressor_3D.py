''' 
vm_predictor1_mlpregressor_3D.py

This script trains a Scikit-Learn MLPregressor to predict the full 3D von Mises stress distribution
...
param_dist = {
    'mlpregressor__hidden_layer_sizes': [(size, size, size, size) for size in range(50, 210, 10)],
...
# model = joblib.load('3D_mlpregressor')
training_start_time = time.perf_counter()
model.fit(x_train, y_train_scaled)
training_end_time = time.perf_counter()
# joblib.dump(model, '3D_mlpregressor')

model_info = extract_model_info(model)

# model, search_results = perform_randomized_search(model, param_dist, x_train, y_train_scaled, n_iter=30, cv=2)
