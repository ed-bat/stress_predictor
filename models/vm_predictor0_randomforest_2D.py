''' 
vm_predictor0_randomforest_2D.py

This script trains a Random Forest regressor to predict the 2D von Mises stress distribution on a previously specified cross-section.
...
def extract_model_info(model):
    """
    Extracts hyperparameter information from the model for use in the results plot.
...
def evaluate_model_performance(model, x_train, y_train, x_valid, y_valid, y_scaler):
    ...
    Prints the RMSE in original scale and plots predictions vs actual values.
    ...
    - y_scaler: Scaler object used for target variable scaling.
    """
    ...
    plot_predictions(y_valid, y_valid_pred, model_info) #Plot performance
...
def final_model_performance(model, x_test, y_test, y_scaler, start_time, training_start_time, training_end_time):
    ...
    """
    prediction_start_time = time.perf_counter() # Record the time at beginning of prediction
    ...
    prediction_end_time = time.perf_counter() # Record the time at end of prediction

    # Calculate the RMSE in the original scale
    test_rmse = np.sqrt(mean_squared_error(y_test, y_test_pred))

    errors = abs(y_test - y_test_pred.ravel())
    max_error = errors.max() # Maximum absolute error

    formatted_test_rmse = format(test_rmse / 1e6, '.4f')  # RMSE in MPa, rounded to 4 decimal places
    test_rmse_percentage = format(test_rmse / y_test.mean() * 100, '.4f')  # RMSE as a percentage to 4 decimal places
    formatted_max_error = format(max_error/1e6, '.4f') # Max error to 4 decimal places
...
# model = joblib.load('2D_randomforest')

training_start_time = time.perf_counter()
model.fit(x_train, y_train_scaled)
training_end_time = time.perf_counter()
# joblib.dump(model, '2D_randomforest')
